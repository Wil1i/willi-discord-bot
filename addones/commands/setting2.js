    ...
    execute(client, message){
        ...
        if(key == "set"){
            ...
            if(availableSetting.hasOwnProperty(subKey)){
                ...
                embed.setDescription(`**${subKey}** successfully set to **${value}**`)
                
            }else{
                
                let avSettings = ""
                for(i,v of availableSetting){
            ...
            if(availableSetting.hasOwnProperty(subKey) && db.has(`${subKey}_${message.guild.id}`)){
            ...
            embed.setDescription(`**SYNTAX** : ${db.get("prefix")}${this.name} [Key (set , delete , reset)] [SubKey (join , left , channel-delete)] [Webhook Link]`)
        }
