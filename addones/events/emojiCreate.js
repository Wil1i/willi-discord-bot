...
module.exports = {
  ...
  async execute(client, emoji) {
    ...
    let Entry = NaN;
    try {
      const AuditLogFetch = await emoji.guild.fetchAuditLogs({
        limit: 1,
        type: "EMOJI_CREATE",
      });
    ...
    if (db.has(`color_${emoji.guild.id}`))
      color = db.get(`color_${emoji.guild.id}`);
    ...
    const embed = new Discord.MessageEmbed()
      ...
      .setDescription(
        `**User ${Entry.executor} added emoji ${emojiAddress} to server**.`
      )
