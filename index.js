...
console.log(red(`[CONFIG]`), green("Importing librarys and config first files..."))
...
const commandsFile = fs.readdirSync("./addones/commands").filter(file => file.endsWith(".js"))
const eventsFile = fs.readdirSync("./addones/events").filter(file => file.endsWith(".js"))
const componentsFile = fs.readdirSync("./addones/components").filter(file => file.endsWith(".js"))
...
for(const command of commandsFile){
    ...
    if(rCommand.execute && rCommand.name){
        ...
        console.log(red(`[REGISTERING]`), green(`Command `), red(rCommand.name), green(" Registered"))
        registeredCommands ++
    }else{
        console.log(red(`[REGISTRING]`), yellow(`Can't register `), red(command), yellow(` command`))
        problemCommands ++
    }
}

console.log(red(`[REGISTERING]`), green(`Successfully registered `), red(registeredCommands), green(" commands and "), red(problemCommands), green(" problems found"))
...
for(const event of eventsFile){
    ...
    if(rEvent.execute && rEvent.name){
        ...
        console.log(red(`[REGISTERING]`), green(`Event `), red(rEvent.name), green(" Registered"))
        registeredEvents ++
    }else{
...
console.log(red(`[REGISTERING]`), green(`Successfully registered `), red(registeredEvents), green(" events and "), red(problemEvents), green(" problems found"))
...
for(const comp of componentsFile){
    ...
    if(rComp.execute && rComp.name){
        ...
        console.log(red(`[REGISTERING]`), green(`Component Response `), red(rComp.name), green(" Registered"))
        registerComponents ++
    }else{
...
console.log(red(`[REGISTERING]`), green(`Successfully registered `), red(registerComponents), green(" component response and "), red(problemComponents), green(" problems found"))
console.log("\n")
...
client.on("messageCreate", message => {
    ...
    console.log(red(`[CMD]`), green("User "), red(message.author.tag), green(` => `), red(command)) // Log commmand in console

    const commandDetail = client.commands.get(command)

    // if user is developer of bot, command going to run without any rules
    if(message.author.id == config.developerID) return commandDetail.execute(client, message);
    ...
    if(commandDetail.permission){
        const hasPermission = client.events.get("permission").execute(message, commandDetail.permission.toUpperCase())
    ...
    if(commandDetail.rank && commandDetail.rank == "administrator" && rank !== "administrator") return;
    if(commandDetail.rank && commandDetail.rank == "admin" && rank !== "administrator" && rank !== "admin") return;
